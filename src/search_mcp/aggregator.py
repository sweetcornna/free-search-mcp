...
from .engines import ENGINES, SearchResult, get_engine
...
def _key(query: str, engines: list[str], max_results: int) -> str:
    raw = json.dumps({"q": query, "e": sorted(engines), "n": max_results}, ensure_ascii=False, sort_keys=True)
...
async def aggregate_search(
    ...
    cache_key = _key(query, engine_names, n)
    ...
    async def run(name: str) -> tuple[str, list[SearchResult] | Exception]:
        ...
        try:
            return name, await engine.search(query, n)
